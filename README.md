...
Email export normally re-verifies previously-exported ids against the target before trusting them, and — for any local email not yet in that id cache — falls back to fetching the target's whole Message-ID index to check whether it already exists there under an id the cache doesn't know about. Both are network round trips proportional to mailbox size, so on a large, actively growing mailbox a rerun with even a handful of new messages would pay the cost of a full target scan every time.

To avoid that, each export records the target's JMAP Email `state` token (an opaque marker the server changes whenever any Email object in the account is created, updated, or destroyed) after finishing cleanly. On the next run, if the target's current state still matches what was recorded, that's server-confirmed proof that nothing changed in between: every cached id is still valid, and any local email not yet in the cache provably isn't on the target either, since it couldn't have gotten there without moving the state. Both checks are then skipped automatically, with no flag needed, and no risk — this isn't an assumption, the server vouches for it. The recorded token is cleared the instant a run decides to rely on it, before doing anything else, and only replaced with a fresh one once that run finishes cleanly, so an interrupted run leaves nothing behind for a later run to (wrongly) trust. Whenever there's no recorded state, or it doesn't match (e.g. the target account is independently receiving mail, such as during a dual-delivery cutover), export falls back to the safe verify/rebuild behavior described above, same as always.

`--assume-not-deleted-in-destination` is a manual override for that fallback case: it skips both checks unconditionally, without the server's proof. Cached ids are trusted outright, and anything not yet cached is created with no dedup search against the target at all. This is fast, but a message created by a run that was interrupted after creating it but before caching its id (a crash, a network drop) would be recreated as a duplicate on a later run — id cache writes happen incrementally, as each batch completes rather than once at the end, specifically to bound (not eliminate) how much an interruption can affect. It also won't notice an email deleted on the target out-of-band. Use it only when you know the target isn't independently changing in ways that matter, and the state-based check above doesn't apply.

Independently of any of this, an email that was deleted at the *source* since the last export (and thus dropped from the archive on the next import) is always detected via a local cache diff and offered for deletion on the target through the normal `--prune` flow.
